...
class ExamenController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $query = Examen::with(['tipoConcurso', 'subpruebas'])
                ...
                ->when($request->filled('search'), function ($q) use ($request) {
                    ...
                    $q->where(function($query) use ($texto) {
    ...
    public function store(StoreExamenRequest $request)
    {
        try {
            ...
            $examenData = $request->only([
                'codigo_examen', 'titulo', 'idTipoConcurso', 'descripcion', 'tipo_acceso', 'estado',
                'tiempo_limite'
            ]);
            ...
            // Asignar el usuario administrador que crea el examen
            $usuarioActual = Auth::user();
            if ($usuarioActual) {
                $examenData['idUsuarioAdmin'] = $usuarioActual->idUsuario;
            } else {
                // Si no hay usuario autenticado, dejar null (aunque no debería pasar con middleware)
                $examenData['idUsuarioAdmin'] = null;
            }
    ...
    public function show($id)
    {
        ...
        // Solo cargar datos básicos: tipoConcurso (necesario para el paso 1)
        ...
        try {
            ...
            if (!$soloFechas) {
                ...
                $updateData = $request->only([
                    'codigo_examen', 'titulo', 'idTipoConcurso', 'descripcion', 'tipo_acceso',
                    'tiempo_limite'
                ]);
    ...
    public function cambiarEstado(Request $request, $id)
    {
        ...
        if ($examen->estado === '0' && $nuevoEstado === '1') {
            ...
            // Cambiando de Publicado a Finalizado
