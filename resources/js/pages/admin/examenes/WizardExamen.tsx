...
const WizardExamen: React.FC = () => {
  ...
  const cargarDatosPaso = React.useCallback(async (paso: number) => {
    if (!examenId) return;

    // Evitar cargar el mismo paso múltiples veces
    if (datosPasoCargadosRef.current[paso]) {
      return;
    }
